...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'restart':
      return {
        ...initalState, questions: state.questions, status: 'ready'
      }
    case 'tick':
      return {
...
export default function App() {
  ...
  useEffect(function () {
    fetch('http://localhost:2323/questions').then((res) => res.json()).then((data) => dispatch({ type: 'dataRecevied', payload: data }))
    ...
    <Main>
      ...
      {status === 'active' && (<>
        ...
        <Footer>
          ...
          <NextButton dispatch={dispatch} answer={answer} index={index} numQuestions={numQuestions} /></Footer> </>
